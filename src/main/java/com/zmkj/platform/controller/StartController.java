...
public class StartController {
    ...
    public String start(String iccid,String d){
        if(iccid != null){
            System.out.println(new Date()+",进入了主页:"+iccid);
        }
        Map<String,Object> card = phoneCardService.getPhoneCardByIccid(iccid);
        Agents agent = null;
        if(card != null){
            Integer aid = (Integer) card.get("aid");
            agent = agentsService.getAgentsById(aid);
            request.getSession().setAttribute("agent",agent);
        }else{
            agent = (Agents) request.getSession().getAttribute("agent");
        }
        ...
        if(iccid != null){
            ...
            request.getSession().setAttribute("phoneCard",phoneMap);
            Map<String,Object> phoneInfo = phoneCardService.getPhoneCardByIccid(phoneMap.get("iccid").toString());
    ...
    public String setmeal(Integer setmeal,String status,String d){
        ...
        if(setmeal == null){
            ...
            int pcid = Integer.parseInt(phoneMap.get("pcid").toString());
            if(pcid == 1){
                order.setSfid(String.valueOf(WFyidong.query(phoneMap.get("iccid").toString())));
            }
            order.setPid(price);
            ...
            try{
                ...
                //扫码支付
                /*Map<String, String> map = service.pay(b, no, "W01", "物联卡流量充值", "暂无备注", "", "123","http://www.kelangtongxin.com/pay/setMealPaySuccess","","","","");
                String payinfo = map.get("payinfo");
                if(payinfo != null){
                    request.setAttribute("payinfo",payinfo);
                    request.setAttribute("tname",tc.getName());
                    request.setAttribute("type",type);
                    request.setAttribute("price",price.getPrice());
                    request.setAttribute("orderid",order.getNumber());
                ...
                //js支付
                Map<String, String> map = service.pay(b, no, "W02", "物联卡流量充值", "暂无备注", request.getSession().getAttribute("openid").toString(), "123","http://"+config.get("domain").toString()+"/pay/setMealPaySuccess","","","","");
                String payinfo = map.get("payinfo");
            ...
            try {
                ...
                if(d != null){
                    return "redirect:http://"+config.get("domain").toString()+"/paying?d=t&tname="+ URLEncoder.encode(tc.getName(),"UTF-8") +"&price="+tc.getPrice()+"&type="+URLEncoder.encode(type,"UTF-8")+"&domain="+agent.getDomain()+"&appId="+mm.get("appId")+"&timeStamp="+mm.get("timeStamp")+"&nonceStr="+mm.get("nonceStr")+"&p="+mm.get("package")+"&signType="+mm.get("signType")+"&paySign="+mm.get("paySign")+"&error="+error;
                }else{
                    return "redirect:http://"+config.get("domain").toString()+"/paying?tname="+ URLEncoder.encode(tc.getName(),"UTF-8") +"&price="+tc.getPrice()+"&type="+URLEncoder.encode(type,"UTF-8")+"&domain="+agent.getDomain()+"&appId="+mm.get("appId")+"&timeStamp="+mm.get("timeStamp")+"&nonceStr="+mm.get("nonceStr")+"&p="+mm.get("package")+"&signType="+mm.get("signType")+"&paySign="+mm.get("paySign")+"&error="+error;
                }
...
//        MemCache.put("website_class",pclassService.getPclass(system_admin.getId(),agent.getId()));
    ...
    public String wxResult(String code,String state,@PathVariable String diy){
        ...
        paramList.add(new BasicNameValuePair("appid","wxe4d148622d2837f6"));
        paramList.add(new BasicNameValuePair("secret","232b814071205815e7e5586614aea6d9"));
        ...
        return "redirect:http://"+configMap.get("domain").toString()+"/index?o="+openid;
    }
    ...
    public String dxQuery(){
        ...
        Map<String,Object> phoneInfo = phoneCardService.getPhoneCardByIccid(p);
        /*if(phoneInfo.get("phone") == null){
        ...
        switch (pcid){
            case 1:
                double tempData = WFyidong.query(iccid);
                ...
                if(date.getMonth() != date2.getMonth()){
                    String upData = stringRedisTemplate.opsForValue().get("wfyd:"+phoneInfo.get("iccid").toString()+lastStr);
                    if(upData != null){
                        tempData = tempData + Double.parseDouble(upData);
                    }
                }
                outData = tempData -Double.parseDouble( map.get("old_data")!=null?map.get("old_data").toString():"0.0" );
                break;
            case 3:
...
*/
        ...
        Map<String,Object> configMap = (Map<String,Object>)MemCache.get("website_config");
        double xujia = Double.parseDouble(commodity.getXuliang());
        ...
        request.setAttribute("startenddata",phoneCardService.getStartEndDate(commodity.getDay(),map.get("iccid").toString()));
        return "query";
    }
